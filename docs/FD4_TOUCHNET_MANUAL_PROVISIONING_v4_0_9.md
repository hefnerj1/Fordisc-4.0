...
FORDISC 4.0.9 also accepts the corresponding supported plan aliases so that an administrator may paste an existing Clerk plan slug without silently losing access. Underscores and hyphens are normalized.
...
No manual action is required to stop access at the end of the term. The backend evaluates `valid_until` on authorization refresh and automatically falls back to the next valid entitlement or Fordisc Demo.

## Single-active-session policy

The same Clerk user-level rule applies to Student, Pro, Institution members and administrators, beta testers, and FORDISC administrators:

- Each account belongs to one named person.
- One active FORDISC Clerk session is allowed per user.
- Multiple tabs sharing the same Clerk session are allowed.
- Activating FORDISC in a different Clerk session revokes the user's other active Clerk sessions.
- No device registration or device-count limit is maintained.
- Replacing a computer does not require entitlement changes; the user signs in on the replacement device.
- If an account or email is no longer recoverable, verify the purchaser/member before provisioning a replacement Clerk account and deactivate/remove the inaccessible account as appropriate.
- For Institution access, device replacement does not consume a new seat; replacing the named person does. Institution administrators remain responsible for member invitations/removals.
- If compromise is suspected, direct the user to reset the password and revoke/disable access as appropriate.

The backend uses Clerk's session API to verify the current session and revoke competing sessions. FORDISC does not maintain a device registry or persistent session-history table.
...
- Final Terms, privacy notice (including Clerk session-control disclosure), refund/cancellation policy, and acceptance process approved.

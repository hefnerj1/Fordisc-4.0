...
FORDISC 4.0.8 also accepts the corresponding supported plan aliases so that an administrator may paste an existing Clerk plan slug without silently losing access. Underscores and hyphens are normalized.
...
## Clerk Organization seat display

When an Organization administrator opens **Manage Institution**, FORDISC synchronizes Clerk's actual Organization membership limit to at least the active 15-seat allocation. This is what controls Clerk's built-in “x of y seats used” display and invitation cap. Larger approved allocations are not reduced.

...
- **Fordisc Demo** may be opened from the signed-out landing page by selecting **Try the Fordisc Demo**. No sign-in or account is required. It can run locked, server-controlled module test cases and view Results and Graphs. It does not provide measurement entry, arbitrary analysis, case storage, imports, Analysis Options, Extended Results, Help, reports, or the Howells Reference Group Map.
- **Fordisc Student** provides FDB and Postcranial Measurement Entry, Results, and Graphs for eligible verified students. Within Postcranial, DFA and stature are run as separate analyses.
- **Fordisc Pro** provides full individual access.
- **Fordisc Institution** provides full access to 15 named users in an active institutional Organization. Additional named users may be added; pricing is to be determined.
  When an Organization administrator opens **Manage Institution**, FORDISC synchronizes Clerk's actual Organization membership limit with the active entitlement. The standard institutional entitlement therefore displays and enforces 15 available named seats. An approved Organization-level `seat_limit` above 15 represents additional seats; an older value below 15 is raised to the current included allocation.
- Approved beta testers and administrators may receive full access through explicit account metadata.

The signed-in Plans page reports when a manual subscription record is present but cannot grant access, including an unrecognized tier, inactive or expired dates, the wrong Clerk scope, or an unmet Student email-verification requirement. Select **Refresh access** after an administrator changes Clerk metadata.

For manual TouchNet provisioning, the recommended tier values are `student`, `pro`, and `institution`. FORDISC also recognizes the supported legacy plan aliases `fd4_student_access`, `fd4-student-access`, `fordisc-student`, `fd4_app_access`, `fd4-app-access`, `fordisc-pro`, `institutional`, `fordisc-institution`, and `fordisc-institutional`. Underscores and hyphens are normalized before the authorization check.

The signed-out opening page also provides a separate **Try the Fordisc Demo** button for the no-account demonstration. The opening and Plans pages show one **Purchase Access Through UTK** button. It opens the University of Tennessee TouchNet store. After a completed order is verified, the FORDISC username and password are supplied by email; account setup may take up to 24 hours. During the invitation-only launch, an administrator provisions the corresponding Student, Pro, or Institution entitlement in Clerk.
...
The signed-out opening page and the in-application start screen now keep the entry experience focused on access and beginning an analysis. The contributor roster is maintained in this Help file and in the public FORDISC project repository rather than being repeated on those screens. The order below is intentional and should be preserved:
...
Choose **Begin FORDISC** on the start screen to enter the application. Within the application, the FORDISC logo in the upper-left corner acts as a home control: selecting it clears the current workspace and returns to the tier-appropriate default screen.
...
FDB, Howells, and Postcranial show a compact **Reference Groups** panel above Measurement Entry. All cranial and Postcranial DFA group selection is completed in that panel; the former expanded Reference Group Selection tab has been retired. On the Postcranial screen, the panel is labeled **DFA Reference Groups** because those selections do not control stature estimation.
...
The **Howells Reference Group Map** tab appears after Help for full-access users. It reproduces the complete Howells location map from the FORDISC 3 Help file in a centered, responsive card. The map is displayed without cropping and scales down with the available screen width. The optional **Howells Samples and Codes** directory lists the current FD4 Howells sample codes and documented source areas.
...
- Complete jackknifed VCVM summaries and matrices, when the explicit advanced option is selected (calculated with the same exact one-case deletion update used by the diagnostics)
...
FORDISC calculates only the Extended Results sections selected in Analysis Options. Complete jackknifed VCVM output is off by default because a routine LOOCV analysis can otherwise generate one full covariance matrix for every reference individual. Selecting that advanced option restores the complete output for the current run; leaving it off does not change the classification, probabilities, ranked typicality, outlier detection, classification matrix, or other ordinary Results.

## 15. Graphs

DFA graphs are generated from the completed analysis and do not rerun the model. For stature, the first-ranked equation graph is returned with the analysis; a lower-ranked equation graph is fitted only when that equation is selected.
...
The selected-equation graph shows reference points, the fitted regression line, prediction interval, and case. The first-ranked equation graph is generated with the initial stature result. Selecting another ranked equation requests only that graph rather than pre-generating graphs for every displayed equation. The supplemental interval plot compares available stature estimates and intervals. Both graphs use the stature display units selected in Analysis Options or, for Fordisc Student, in the Postcranial stature settings.
...
Postcranial Measurement Entry uses the approved compact horizontal anatomical layout. One shared postcranial measurement record supports two **separate** analyses. The screen and header provide independent actions for **Run Postcranial DFA** and **Estimate Stature**. The action cards are intentionally concise; their readiness line reports whether each operation can run.

This restores the FORDISC 3 workflow: stature may be calculated directly from the checked, entered postcranial measurements and the selected stature reference settings. A Postcranial DFA does not need to be run first. If a current Postcranial DFA result exists, its top classified group may be used as an optional stature-group shortcut; otherwise choose **Any** or a manual stature group.
...
Return to the signed-out opening page and choose **Try the Fordisc Demo**. The browser should open the `/try` workspace without an account. Fordisc Demo can access only the locked Module Test Cases, Results, and Graphs. A temporary busy or rate-limit message may appear if the public demonstration is processing several requests; wait briefly and try again.

### Manage Institution still shows the wrong seat total

Open **Manage Institution** while the institution is active and while signed in as an Organization administrator. FORDISC synchronizes the actual Clerk Organization membership cap before remounting Clerk's management panel. The standard entitlement should then display 15 available named seats. A stale Organization metadata value of 10 is raised to 15 automatically; values above 15 remain available for approved additional seats. If synchronization cannot be completed, the page shows a seat-limit warning; confirm that the production Clerk secret key is configured and that the Organization has an active institutional entitlement.
...
A Howells comparison using many reference groups with **LOOCV** can take longer than a smaller comparison because every reference individual must be evaluated under leave-one-out calculations. Keep the analysis window open while **Processing analysis** is displayed. FORDISC 4.0.6 performs ranked typicality and reference-outlier detection in one shared exact one-case-deletion pass, reuses fitted covariance and canonical products, and does not generate complete jackknifed VCVM matrices unless the advanced option is selected.

Only one analytical request may run for a named user at a time. A second click or analysis request from another tab receives a message asking the user to allow the current analysis to finish. This is a computational resource safeguard; it is not the future account-sharing or single-Clerk-session policy.

The server retains a finite calculation safety limit. The packaged default is 900 seconds and may be adjusted by the deployment administrator. If that limit is reached, FD4 returns a clear calculation-limit message and no partial result is saved; it no longer exposes the former raw 500 error caused by the fixed 180-second bridge timeout. Server logs record workload, major analytical stages, serialization time, response size, and total bridge time for support diagnostics without intentionally logging case measurements or report contents.
...
- Final approval of the web Terms and Conditions, renewal, refund, and the planned single-active-Clerk-session/account-sharing policy
- Automated TouchNet-to-Clerk subscription provisioning
- Additional stature-equation and reporting review beyond the restored independent workflow

## 26. Citation and support

Until a final FD4 citation is approved, analytical documentation should identify the application simply as **FORDISC 4.0** and record the module, reference data, variables, groups, and options used. The technical build number remains available through `/health` for support and reproducibility, while internal R-engine identifiers are not shown in routine reports. The historical FORDISC 3 citation remains relevant to the lineage of the methods:
...
The product is identified throughout the user interface, reports, and Run Log output as **FORDISC 4.0**. Technical build identifiers remain available through the `/health` endpoint and release documentation but are not displayed in ordinary analytical reports.

...
Institution access belongs on the Clerk Organization, not on the purchaser’s individual user record. The initial plan includes 10 named users. The Organization membership limit should be set to the purchased total when Clerk configuration permits it.
...
FORDISC 4.0.2 also accepts the corresponding supported plan aliases so that an administrator may paste an existing Clerk plan slug without silently losing access. Underscores and hyphens are normalized.

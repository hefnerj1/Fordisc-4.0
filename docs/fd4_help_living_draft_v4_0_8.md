...
### Institution page still shows 10 seats

Open **Manage Institution** as an Organization administrator after the current deployment is healthy. FORDISC synchronizes the active Clerk Organization membership cap to at least 15 named seats and then reloads the Organization profile. A larger approved allocation is preserved. If the count does not update, confirm that the production `CLERK_SECRET_KEY` is configured and that `FD4_CLERK_SYNC_ORGANIZATION_SEAT_LIMIT=1`.
...
- **4.0.8:** Corrected the failed mixed-tree frontend rollback build; restored the accepted 4.0.5 analysis and stature-graph behavior; retained the 15-seat Clerk Organization synchronization.

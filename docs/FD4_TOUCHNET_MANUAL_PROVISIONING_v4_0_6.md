...
FORDISC 4.0.6 also accepts the corresponding supported plan aliases so that an administrator may paste an existing Clerk plan slug without silently losing access. Underscores and hyphens are normalized.
...
## Institution seat-limit synchronization

FORDISC 4.0.6 synchronizes the active Clerk Organization membership limit when an Organization administrator opens **Manage Institution**. The purchased `private_metadata.fd4_subscription.seat_limit` remains authoritative. When an active Institution record does not specify a seat limit, the included-plan default is 15.

The synchronization changes Clerk's actual Organization membership limit; it does not merely replace the displayed text. The Organization administrator may then invite named members through Clerk until the purchased limit is reached. Additional seats should be provisioned by updating both the Organization subscription metadata and the approved Clerk limit.

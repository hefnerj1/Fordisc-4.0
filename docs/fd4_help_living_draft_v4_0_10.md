...
One FORDISC 4.0 case can contain cranial and postcranial information. FDB, Howells, and Postcranial retain independent measurement-use and group-selection states within the same case.
...
The Demo Cases demonstrate the workflow. They are not interpretive standards for unknown remains.
...
FDB, Howells, and Postcranial show a compact **Reference Groups** panel above Measurement Entry. All cranial and Postcranial DFA group selection is completed in that panel. On the Postcranial screen, the panel is labeled **DFA Reference Groups** because those selections do not control stature estimation.

Measurements shared by FDB and Howells use one cranial case record. Entering a shared cranial value in one module makes it available in the other. Postcranial values are stored in the same overall case. From that shared postcranial record, the user chooses either **Run Postcranial DFA** or **Estimate Stature**; one analysis is not a prerequisite for the other.
...
Select **Estimate Stature** to calculate stature without running DFA. Stature uses the original entered measurements and its own reference source, group mode, prediction interval, birth-year limits, missing-birth-year setting, maximum terms, sort order, and display units.
...
The analysis report includes the case summary, analysis settings, warnings, measurement checks, classification matrix, group relationship table, stature results in the currently selected display units, and available graph snapshots. It identifies the application simply as **FORDISC 4.0**. Browser PDF output uses one FORDISC page-number system; disable the browser print dialog's own Headers and footers if it would add a second unrelated page-number series.

### Run Log PDF

The Report page contains one reporting panel with three controls: **Download Analysis Report**, **Print / Save as PDF**, and **Export Run Log as PDF**. The Run Log PDF preserves the chronological Results-page record rather than converting it into a spreadsheet-style table.


## 18. Import Data and custom reference datasets

Import Data supports preview and analysis of compatible CSV, TXT, DBF/dBase, and unencrypted Advantage ADT tables.

FORDISC detects likely ID and grouping fields, matches recognized FORDISC measurement abbreviations, reports groups and missingness, and can apply a compatible first row as the current case.

The FORDISC 4.0 custom-reference workflow is narrower than the fully generic FD3 CustomDB screen. FD3 could analyze arbitrary numeric fields with arbitrary text grouping fields. Do not assume that every non-FORDISC numeric column in an imported file is available for analysis.

## 19. Core FORDISC analytical principles
...
## 20. Measurement abbreviation reference
...
## 24. Citation and support

Until a final FORDISC 4.0 citation is approved, analytical documentation should identify the application simply as **FORDISC 4.0** and record the module, reference data, variables, groups, and options used. The historical FORDISC 3 citation remains relevant to the lineage of the methods:

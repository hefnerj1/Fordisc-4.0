# FORDISC 4.0 Help File - Living Draft v1.0.15
...
FD4 uses individual Clerk accounts for sign-in. Institutional memberships use named members in a Clerk Organization. The initial production release is invitation-only.

- **Fordisc Free** can run server-controlled module test cases and view Results and Graphs. It does not provide measurement entry or case storage.
...
Choose **About FORDISC / Contributors** to open the contributor-history panel. Choose **Begin FORDISC** to enter the application.
...
The **Reference Group World Map** tab appears after Help for full-access users. It reproduces the Howells location map from the FORDISC 3 Help file and overlays approximate source-area markers for the FDB samples. The tab also lists current FD4 Howells and UTK comparison-group codes.

Map locations are geographic orientation only. They do not represent biological, social, or population boundaries and are not a substitute for a scientifically appropriate comparison question. FDB markers are approximate because several FDB samples encompass broad regions rather than a single collection site.

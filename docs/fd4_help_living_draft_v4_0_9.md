...
Each paid or invited FORDISC account is assigned to one named user. Do not share an individual account, username, password, authentication code, or sign-in session. A named user may access FORDISC from multiple computers or devices, but only one Clerk session may be active for that user at a time. Multiple tabs that share the same Clerk session are allowed. Activating FORDISC in another browser or device ends the prior active Clerk session. This same rule applies to Student, Pro, every named Institution member (including Institution administrators), beta testers, and FORDISC administrators. Fordisc Demo is anonymous and is not subject to this Clerk session rule.

FORDISC does not use a registered-device list, device cap, hardware identifier, browser fingerprint, or IP-based device record for this policy. The application transiently uses the Clerk user ID, Clerk session ID, and current session status to enforce the one-active-session rule. It does not create a separate persistent FORDISC session-history database. The draft web Terms and Conditions remain subject to University of Tennessee review and approval.
...
FORDISC allows one active Clerk session per named user. If the same account activates FORDISC in another browser, private/incognito window, computer, or device, the newer activation ends the prior active Clerk session. Multiple tabs within the same Clerk browser session remain allowed. Select **Return to sign in** and sign in again only if you intend that browser/device to become the active FORDISC session. If you did not initiate the other activation, reset your Clerk password and contact subscription support if unauthorized access is suspected.

### Replacing or losing a computer or device

No device-transfer or device-removal request is required. Sign in to FORDISC from the replacement device. That activation becomes the active FORDISC session and ends the prior active Clerk session. If the lost device may have been accessible to someone else, reset the account password as a precaution. Institution administrators should remove a member from the Clerk Organization when the person no longer requires institutional access; a replacement computer does not require a new institutional seat.

### Session verification is temporarily unavailable

Authenticated FORDISC analysis is fail-closed when Clerk session status cannot be confirmed. If FORDISC reports that session verification is temporarily unavailable, wait briefly and try again. The analysis is not allowed to proceed without confirmation of the active Clerk session. Fordisc Demo is unaffected because it does not use a Clerk account.
...
- Final approval of the web Terms and Conditions, privacy notice, renewal, and refund policies

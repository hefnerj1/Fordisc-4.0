...
- **International Crania** - cranial discriminant-function analysis using the International reference dataset. This module is available only with Fordisc Pro and Fordisc Institution access.
- **Postcranial** - two independently run analyses from one shared postcranial measurement record: Postcranial DFA and Stature Estimation.
...
- **Scientific Lead & FORDISC 4.0 Director:** Joseph T. Hefner
- **Computational Engine:** Michael W. Kenyhercz, Joseph T. Hefner, and Richard L. Jantz
- **Original FORDISC Developers:** Richard L. Jantz and Stephen D. Ousley
- **Statistical Consultation:** Richard L. Jantz, Michael W. Kenyhercz, and Valerie Sgheiza
- **Beta Testing:** Valerie Sgheiza, Christopher Rainwater, Andrea Zurek-Ost, Carolyn Isaac, Alexandra Klales, Phoebe Stubblefield, Laura Manthey, Yangseung Jeong, and M. Ancern
- **Translations:** Luis L. Cabo and Kelly Kamnikar
- **Institutional Support:** University of Tennessee, Knoxville, and the UTK Forensic Anthropology Center

Graduate students and additional researchers will be added as the contributor history is completed.

## 3. Case-based workflow

One FORDISC 4.0 case can contain cranial and postcranial information. FDB, Howells, International Crania, and Postcranial retain independent measurement-use and group-selection states within the same case. Older `.fd4case` files remain compatible; they simply have no saved International state until that module is configured and the case is saved again.
...
6. Run the analysis. On FDB and Howells Measurement Entry, the same **Run Analysis** action is available in the header and again below the measurement table so a long page does not require scrolling back to the top.
7. Review Results, Extended Results, and Graphs.
8. Record interpretation in Notes / Run Log.
9. Export the Case Report or the complete Run Log PDF needed for documentation.
...
**Open FD3 Case** is for a legacy individual case, not a custom reference dataset. Compatible inputs include FD3-shaped ADT, DBF, CSV, and TXT files. FORDISC previews the restored Case ID, measurements, groups, readable comments, and suggested module before applying the case.

Some comments from older FD3 files may not be recoverable automatically. Review Case Notes after opening an FD3 case.

### Open Demo Case

The Demo Cases demonstrate the workflow. They are not interpretive standards for unknown remains. Fordisc Demo does not include an International Crania server-owned test case.
...
FDB, Howells, International Crania, and Postcranial show a compact **Reference Groups** panel above Measurement Entry. All cranial and Postcranial DFA group selection is completed in that panel. On the Postcranial screen, the panel is labeled **DFA Reference Groups** because those selections do not control stature estimation.

Measurements shared by FDB, Howells, and International Crania use one cranial case record. Entering a shared cranial value in one module makes it available in the others, while measurement-use and group selections remain independent by module. Postcranial values are stored in the same overall case. From that shared postcranial record, the user chooses either **Run Postcranial DFA** or **Estimate Stature**; one analysis is not a prerequisite for the other.
...
When **Classify Case** is unchecked, FORDISC performs a group-only analysis. In this mode, **Use all** selects all available eligible non-calculated measurements, even when the case fields are blank. No unknown case is classified and no case marker is shown on the graph.
...
FORDISC displays supported calculated cranial angles and related auxiliary values as read-only fields. In the standard FDB workflow, calculated and auxiliary variables are not automatically included in the default candidate pool.

## 5. Measurement range checking
...
## 6. Measurement Checks symbols, colors, and importance columns

The Measurement Checks table follows the FD3 diagnostic rules. Each current-case value is compared separately with **every selected group's own mean and standard deviation**. FORDISC does not average the group means or standard deviations to create the check symbol. A sign is added only when the case crosses the same boundary for all selected groups; the original FD3 0.1-unit tolerance is retained.
...
## 7. Reference coverage and sample-size advisories

A measurement may be valid but sparsely recorded in one or more selected reference groups. Before standard FDB or Howells analysis, FORDISC removes selected variables that do not have more than eight complete reference rows in every selected group. Removed variables are listed in Results and reports.
...
### Howells Reference Group Map

The **Howells Reference Group Map** tab appears after Help for full-access users. It reproduces the complete Howells location map from the FORDISC 3 Help file. The optional **Howells Samples and Codes** directory lists the current FORDISC 4.0 Howells sample codes and documented source areas.
...
Postcranial DFA uses the supported postcranial reference groups selected in the compact DFA Reference Groups panel. Stature reference selection is separate from Postcranial DFA group selection, and the selected DFA groups do not filter the stature reference sample.

## 9. Analysis Options

### Classify Case

When checked, FORDISC classifies the current case using the entered selected measurements. When unchecked, FORDISC compares the selected reference groups without classifying an unknown. Blank selected case measurements are allowed in group-only mode.

### Classification Rate Estimation

- **LOO (Jackknife; 1, N-1)** removes each reference individual in turn, recalculates the discriminant function, and classifies that omitted individual. It is the default and the preferred FORDISC estimate of classification performance.
...
FORDISC uses a fixed complete-case cohort during Stepwise variable selection:
...
4. After the variables are selected, FORDISC performs the final query using only the retained variables. Final group sample sizes may therefore be larger than the sample sizes used during variable selection.
...
## 11. Warnings, errors, and color legend

FORDISC uses color as a secondary cue; important conditions also have text labels or symbols.
...
- **Graph colors and marker shapes** - identify reference groups only. They do not indicate rank, confidence, or statistical significance.
...
Large tables scroll horizontally, with the first column remaining visible.
...
Typ. R ranks the case D-squared against leave-one-out own-group D-squared values. FD3's legacy insertion rule uses a strict-greater comparison and one baseline array position. In FORDISC 4.0 this is reproduced as one plus the number of reference distances less than or equal to the case distance, capped at the group sample size. The denominator is the group sample size plus one.
...
Extended Results uses the same horizontal scrolling behavior for large tables.
...
- Jackknifed VCVM output
- Stepwise selection log and selection-sample information
- All stature equations

The displayed multigroup eigenvalues use the FD3 equal-prior group-centroid normalization. The displayed percentages of total canonical variation are unchanged.
...
When at least three canonical axes are available, the 3D plot shows reference scores, centroids, and the case in CV1-CV2-CV3 space. Rotation changes only the view. Use the Rotate and Zoom buttons for keyboard or single-click control; pointer drag and wheel controls remain available.
...
## 15. Postcranial DFA and stature

Postcranial Measurement Entry presents measurements in a compact horizontal anatomical layout. One shared postcranial measurement record supports two **separate** analyses. The screen and header provide independent actions for **Run Postcranial DFA** and **Estimate Stature**. The readiness line reports whether each operation can run.

Stature may be calculated directly from the checked, entered postcranial measurements and the selected stature reference settings. A Postcranial DFA does not need to be run first. If a current Postcranial DFA result exists, its top classified group may be used as an optional stature-group shortcut; otherwise choose **Any** or a manual stature group.

### Postcranial DFA

Postcranial DFA follows the same variable, group, classification-rate, Stepwise, outlier, probability, and reporting principles as cranial DFA. The compact **DFA Reference Groups** panel controls only this analysis.

Running Postcranial DFA does not automatically run stature estimation. Conversely, running stature estimation does not calculate a classification matrix, posterior probabilities, typicality probabilities, reference outliers, or other DFA output.

### Stature estimation

Select **Estimate Stature** to calculate stature without running DFA. Stature uses the original entered measurements and its own reference source, group mode, prediction interval, birth-year limits, missing-birth-year setting, maximum measurements per equation, sort order, and display units.

The stature-group choices are independent of the DFA comparison groups:

- **Any** is the default and does not require a previous DFA.
- **Manual group** uses the selected supported FD3 stature group: BF, BM, WF, WM, or HM.
- **Current top Postcranial DFA group** is available only when a DFA has been run with the current postcranial measurements, checked variables, groups, and DFA settings. If the current top group has no matching FD3 group-specific stature option, FORDISC uses **Any** and reports that resolution.

As in FORDISC 3, stature considers only the checked, entered stature-compatible length or height variables: CALCXL, CLAXLN, FEMBLN, FEMXLN, FIBXLN, HUMXLN, INNOHT, RADXLN, SACAHT, SCAPHT, TIBXLN, ULNXLN, and ULNPHL. Other checked postcranial measurements remain available to DFA but are not submitted to the stature regression generator.

DFA group selection, Stepwise selection, transformations, reference-outlier exclusions, and classification results do not alter the stature reference pool. The stature reference source and stature group controls determine that pool independently.
...
## 16. Notes, Run Log, and bench notes

Case Notes are saved in the `.fd4case` file. The Run Log follows the FORDISC 3 running-log model: every completed analysis is appended chronologically as **RUN 1, RUN 2,** and so on, and the complete Results page for that run is repeated in the log. A Postcranial DFA and a stature estimate are recorded as separate runs. DFA entries retain their classification material; stature entries retain their stature settings, selected compatible measurements, estimates, and display units.

The **Export Run Log as PDF** control opens a print-ready document that repeats the same Results-page presentation for every recorded run, with each entry identified as **RUN 1, RUN 2,** and so forth. Choose **Save as PDF** in the browser print dialog. Wide Run Log tables use a page size that preserves all group columns. Case Notes and the Run Log do not replace the analyst's interpretation or agency documentation requirements.

## 17. Reports and exports

### Case Report and PDF

The **Case Report** summarizes the latest successful result available for each completed analytical lane in the case: FDB Cranial DFA, Howells Cranial DFA, International Crania DFA, Postcranial DFA, and Stature Estimation. A lane is omitted when it has not been run. If an analysis is repeated, the Case Report uses the most recent successful result for that lane; earlier and intermediate runs remain in the Run Log.

Each DFA summary records the completed run, measurements and reference groups used, transformation and Stepwise status, classification results, typicality probabilities, classification accuracy, removed measurements, reference-outlier counts, and important warnings when available. The stature summary records the measurements used, stature reference source and group, best-ranked equation, point estimate, prediction interval, display units, and number of equations evaluated.

The Case Report also includes the graph snapshots retained with the latest successful result for every completed analytical lane. Graphs for earlier analytical lanes appear under **Case Analysis Graphs**; the current active result retains its full detailed tables and graph snapshots under **Detailed Analysis**. Stature reporting includes both the selected-equation graph and the supplemental estimate-interval graph when available. Saving and reopening a `.fd4case` file preserves the combined Case Analysis Summary and its retained graph snapshots through the saved Run Log.

The report identifies the application simply as **FORDISC 4.0**. Browser PDF output uses one FORDISC page-number system; disable the browser print dialog's own Headers and footers if it would add a second unrelated page-number series.

### Run Log PDF

The Report page contains one reporting panel with two controls: **Download Case Report** and **Export Run Log as PDF**. **Download Case Report** opens a print-ready view; choose **Save as PDF** in the browser print dialog. The Case Report provides the latest case-level summary; the Run Log PDF preserves every completed run in chronological order rather than replacing earlier results.


## 18. Core FORDISC analytical principles
...
## 19. Measurement abbreviation reference

The definitions below follow the terminology used in the FD3 Help and standard FORDISC measurement references.
...
### Fordisc Demo asks for a sign-in

Return to the signed-out opening page and choose **Try the Fordisc Demo**. Fordisc Demo can access only the built-in Demo Cases, Results, and Graphs. A temporary busy or rate-limit message may appear if the public demonstration is processing several requests; wait briefly and try again.

### A FORDISC session ended after signing in elsewhere

FORDISC allows one active session per named user. If the same account activates FORDISC in another browser, private/incognito window, computer, or device, the newer activation ends the prior session. Multiple tabs within the same browser session remain allowed. Select **Return to sign in** and sign in again only when that browser or device should become the active FORDISC session. If you did not initiate the other activation, reset your account password and contact subscription support if unauthorized access is suspected.

### Replacing or losing a computer or device

No device-transfer or device-removal request is required. Sign in to FORDISC from the replacement device. That activation becomes the active FORDISC session and ends the prior session. If the lost device may have been accessible to someone else, reset the account password as a precaution. Institution administrators should remove a member from the institutional account when that person no longer requires access; a replacement computer does not require a new institutional seat.

### Session verification is temporarily unavailable

FORDISC could not verify your session. Wait briefly and try again. Fordisc Demo is unaffected because it does not require an account.

### Analysis will not run

For cranial or Postcranial DFA, confirm that at least the required number of DFA groups is selected, enough measurements are checked, required case values are present when Classify Case is on, measurement checking has been addressed, and enough complete reference records remain.

For stature, no DFA groups are required. Confirm that at least one of the 13 stature-compatible measurements is both checked and entered, that a valid stature group is selected, and that measurement checking has been addressed.
...
A Howells comparison using many reference groups with **LOOCV** can take longer than a smaller comparison because each reference individual must be evaluated under leave-one-out calculations. Keep the analysis window open while **Processing analysis** is displayed. If the calculation limit is reached, no partial result is saved.
...
### The selected language does not update

Select English or Español from the Language control. If the labels do not update, refresh the application and select the language again.

## 23. Citation and support

Cite FORDISC 4.0 as:

Hefner, JT, Jantz RL, and Ousley SD. FORDISC 4.0: Personal computer forensic discriminant functions. University of Tennessee; 2026.

Analytical documentation should also record the module, reference data, variables, groups, and options used. The historical FORDISC 3 citation remains relevant when discussing the lineage of the methods:
...
For subscription and eligibility questions, contact **fordisc.support@gmail.com**. Do not send case measurements or personally identifying case information unless specifically requested through an approved secure channel.
